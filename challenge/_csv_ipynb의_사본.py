...
class MobileNetV3(nn.Module):
    def __init__(self, n_class=100, input_size=32, dropout=0.3, mode='small', width_mult=1.0):
...
class HCGNet(nn.Module):
    def __init__(self, growth_rate=(8, 16, 32), block_config=(6,12,24,16),
                 bn_size=3, theta=0.5, num_classes=10, groups=0):
...
#model = HCGNet_A1().to(cuda)


def HCGNet_A3(num_classes=100):
    return HCGNet(growth_rate=(8, 8, 8), block_config=(5, 5, 5), num_classes=num_classes, bn_size = 7, groups = 8)


def HCGNet_A4(num_classes=100):
    return HCGNet(growth_rate=(3, 9, 18), block_config=(5, 5, 5), num_classes=num_classes, bn_size = 4, groups = 3)
def HCGNet_A6(num_classes=100):
    return HCGNet(growth_rate=(12, 12, 12), block_config=(4, 4, 4), num_classes=num_classes, bn_size = 2, groups = 12)
def HCGNet_A7(num_classes=100):
    return HCGNet(growth_rate=(18, 18, 18), block_config=(2, 2, 2), num_classes=num_classes, bn_size=5, groups=18)

model = HCGNet_A6().to(cuda)
summary_(model, (3, 32, 32), batch_size=1)
path2 = 'D:/User_Data/Desktop/github/2022-DL/challenge/A6_model/model1450.pt'
model.load_state_dict(torch.load(path2))
f = open('D:/User_Data/Desktop/github/2022-DL/challenge/write1.csv','w', newline='')
wr = csv.writer(f)
